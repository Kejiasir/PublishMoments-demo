...
- (void)setAnimationRightTick:(BOOL)animationRightTick {
    ...
    if (animationRightTick) {
        [self.tickImageView setImage:[UIImage imageNamed:MLSelectPhotoSrcName(@"AssetsPickerChecked")] forState:UIControlStateNormal];
    } else {
...
- (UIButton *)tickImageView {
    if (!_tickImageView) {
        ...
        tickImageView.frame = CGRectMake(self.bounds.size.width - 29, -2, 30, 30);
